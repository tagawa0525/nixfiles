...
| `REREVIEW_NEEDED`      | 対応を push したが再レビューを要求していない | **Step 6.2 で要求する**（挙動を変えない修正だけなら 6.3）                                      |
...
**表記・コメント・整形など、挙動を変えない修正だけを push した場合は、再レビューを
依頼せずマージしてよい**。レビュー 1 周には数分とレビュー用トークンがかかり、
typo の修正には見合わない。

- **対象**: ドキュメント、コメント、整形、表記の統一
- **対象外**: ロジック、条件式、API 呼び出し、テストの期待値（実行パスに触れるもの）
- **条件**: `run-checks.sh` が `ALL_OK` で、既存テストが通ること
- **手順**: `ALLOW_UNREVIEWED_HEAD=1 gh pr merge …` でマージし、Step 8 の完了報告に
  「レビューを受けていない変更」として commit hash と要旨を必ず載せる

`decide-next.sh` は差分の意味を判断できないので、この場合も `REREVIEW_NEEDED` を
返す。挙動を変えないかどうかを決めるのはこちら。**迷ったら再レビューを依頼する**。
...
- レビューを受けていない変更: {6.3 で再レビューを省いた場合の commit hash と要旨。なければ省略}
...
- 挙動を変えない修正（表記・コメント・整形）だけなら再レビューを省いてよい（6.3）。
  省いた場合は完了報告に必ず記載する
