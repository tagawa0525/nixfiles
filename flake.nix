...
# =============================================================================
{
  ...
  inputs = {
    ...
    lanzaboote = {
      # nixpkgs で boot.bootspec.enable が削除され、最新リリースの v1.0.0 を含む
      # 全リリースは同オプションを設定するため評価が失敗する。修正コミット
      # （module: don't set bootspec.enable）を含む master のコミットに固定。
      # 修正を含む次のタグリリースが出たらタグ指定に戻す
