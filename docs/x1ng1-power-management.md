...
**2026-09-23 に外した。**導入後最初の固まり (下記「続報 (2026-09-23 夕方)」) は、復帰した後に起きていて記録の対象外だった。さらに強制断後の起動では、RTC が `2001-01-01T00:00:43 UTC` に初期化されていて何も読めなかった (`Magic number: 1:0:0`)。電源投入から 43 秒ほどで読まれていることから、初期化は電源を入れ直した時点で起きている。BIOS が pm_trace の書く 1900 年代の日付を不正とみなして初期化した可能性があるが、確かめていない。デバイス処理の同期化でサスペンドも大きく遅くなるため、代償に見合わないと判断した。`pm_print_times` / `pm_debug_messages` は、タイミングを変えずにデバイスごとの所要時間を残せるので残している。以下は導入時の記録。
...
ssh がつながらないときは、強制断の前に SysRq でカーネルの状態を残す (`hosts/x1ng1/default.nix` で `kernel.sysrq = 184`)。**Alt を押したまま PrtSc を押し、両方押したまま**次のキーを順に押す。
...
- **keyd がキーボードを独占したまま止まっていると、SysRq は効かない。**内蔵キーボードは keyd が独占 (EVIOCGRAB) していて、カーネルの `input_pass_values()` は独占中のデバイスのイベントを独占している側にしか渡さない。独占中は、keyd が仮想キーボードへ送り直した入力だけが SysRq の処理に届く。keyd のプロセスが残ったまま止まっている (ユーザー空間ごと止まっているなど) と、どこからも届かない。一方で keyd が終了していれば、ファイルが閉じられて独占が解除され (`evdev_release()` → `evdev_ungrab()`)、物理キーボードのイベントが直接届く
