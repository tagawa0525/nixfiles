...
`<バス> <デバイス>: hash matches` の行 (例: `pci 0000:08:00.0: hash matches`) が止まったデバイス、`hash matches drivers/base/power/main.c:NNN` の行がそのフェーズ (ソース行) を示す。

| 結果                                 | 解釈                                                     |
| ------------------------------------ | -------------------------------------------------------- |
| `0000:08:00.0` または `0000:00:1c.0` | LTE モデムが犯人。下記「次の手」に進む                   |
| それ以外のデバイス                   | LTE モデム説は外れ。そのデバイスを追う                   |
| デバイスが出ない                     | 全デバイス処理後のプラットフォーム段階 (S0ix) で止まった |
...
```console
# 1 秒以上かかったもの
journalctl -b -k | grep -E "returned 0 after [0-9]{6,} usecs"
```
