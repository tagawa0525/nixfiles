...
| hibernate                        | 以前は 4/4 回ハング。2026-09-23 に 7.2.7 + pm_trace 有効で**初めて成功**。原因は未切り分け |
...
- デバイスの suspend/resume が同期実行になる。カーネルの `is_async()` が `pm_trace_is_enabled()` を見るため。**タイミングが変わるので、ハングが起きにくくなる可能性がある。**起きなくなった場合、それ自体が「非同期実行時の競合」という手がかりになる。実測では、復帰時のデバイス処理に計約 5.3 秒かかった (noirq 2.1 秒 / early 1.2 秒 / 通常 2.0 秒)。無効時はこの集計が出ないため、比較値はない
...
**2026-09-23 に 1 回成功した。**pm_trace の動作確認のために `systemctl hibernate` を実行したところ、以前止まっていたデバイス凍結段階 (freeze / late freeze / noirq freeze) を通過し、イメージの書き込み、電源断、復元まで完了した (`Hibernation image restored successfully`)。以下の「症状」以降は、成功する前の記録である。

以前と比べて少なくとも 3 つの条件が違い、どれが効いたかは切り分けていない。

| 条件                      | 以前 (4 回とも停止)            | 2026-09-23 (成功)            |
| ------------------------- | ------------------------------ | ---------------------------- |
| カーネル                  | 7 月時点の版                   | 7.2.7                        |
| デバイスの suspend/resume | 非同期                         | **同期** (pm_trace の副作用) |
| iosm                      | ロード済み (unbind しても停止) | blacklist 済み               |

2 行目が効いていた場合、ハングの原因は**デバイス処理を非同期で実行したときの競合**ということになる。s2idle の復帰ハング (上記) も同じデバイス処理の段階で起きうるため、同じ原因である可能性がある。切り分けるには、pm_trace を無効 (非同期) に戻して hibernate を試せばよい。止まれば非同期が原因、通ればカーネル更新などの別要因となる。
