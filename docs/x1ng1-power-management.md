...
- **前ブートが正常に終わっていた場合は解釈しない。**起動時の読み出しは毎回走る。pm_trace が無効なら RTC の通常の時刻を誤読した値が出る (導入前の 2026-09-23 10:11 の起動では、ファイル行なしで `acpi device:22: hash matches` と出た)。有効なら最後に成功したレジュームの地点 (多くは `1186`) が出うる
