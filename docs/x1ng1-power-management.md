...
ThinkPad X1 Nano (1st Gen) の電源まわりを実測した記録 (2026-07-28、2026-08-01 / 2026-09-07 追記)。
...
さらに後日 (2026-09-07)「サスペンドから復帰しない」事象が発生し、こちらは**内蔵 LTE モデム (iosm) の初期化失敗**が引き金だった。同じくサスペンド周りのため本文書に含める。
...
| s2idle から復帰しない (2026-09-07) | **LTE モデム (iosm) の初期化失敗時のみ**。iosm を blacklist して対処 |
...
## s2idle から復帰しないことがある (LTE モデムの初期化失敗時)

2026-09-07、AC を抜いて放置した後に**サスペンドから一切復帰しなくなった**。電源ボタン・蓋の開閉・キーボード・タッチパッドのいずれにも無反応で、電源長押しによる強制断以外に手がなかった。
...
journal に残る 13 boot 分で、**iosm の初期化成否とサスペンド復帰成否が完全に一致する**。
...
初期化に失敗した `iosm` は PCI デバイスを掴んだまま IPC ハンドシェイクが完了しない半端な状態で残る。この状態でサスペンドすると復帰処理がハングする。
