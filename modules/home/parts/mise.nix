...
in
{
  ...
  home.sessionVariables = {
    # miseは /etc/NIXOS の存在からNixOSを検出し、配布バイナリは動的リンカが
    # ないと動かない前提で全ランタイムをソースビルドする（all_compile）。
    # しかし本環境には nix-ld があり配布バイナリはそのまま動くため無効化する。
    # Node.jsで数十分かかっていたインストールが数十秒で済む。
    #
    # 設定ファイル（~/.config/mise/config.toml）ではなく環境変数で渡すのは、
    # 同ファイルが `mise use -g` の書き込み先であり、Nix管理下に置くと
    # 読み取り専用になって壊れるため。
    #
    # 配布バイナリが存在しないバージョンではmiseがソースビルドに自動で
    # フォールバックするため、以下のビルド依存設定は引き続き必要。
    # なおpythonの配布バイナリ（python-build-standalone）はGPLのgdbmを
    # 含まないため dbm.gnu だけは使えない。
