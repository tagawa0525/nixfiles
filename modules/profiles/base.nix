...

{
  ...
  # ストレージ (Btrfs)
  # ===========================================================================
  # 月次でスクラブを実行し、チェックサム照合でビット腐敗を早期検出する。
  # 全ホストが単一デバイスBtrfs（サブボリューム分割）のため、データの
  # 自己修復はできないが、破損ファイルの特定によりバックアップからの
  # 復旧を可能にする。対象はデフォルトの "/"（同一FSなので全体をカバー）
  ...
  programs.fish.enable = true; # zellij内で使用

  # ===========================================================================
  # GnuPG
  # ===========================================================================
  # agent.enable で gnupg 本体が systemPackages に入り、GPG_TTY も設定される
  # （パスフレーズ入力に必要）。pinentry はデフォルトの curses を使い、
  # GUI環境では workstation プロファイルで GTK 版に差し替える
  programs.gnupg = {
    agent.enable = true;
    dirmngr.enable = true; # 鍵サーバーからの公開鍵取得に必要
  };
  ...
  environment.systemPackages = with pkgs; [
    ...
    zat # tree-sitter製アウトライン表示。pub APIと行番号を一覧（Rust/C#等14言語）
  ...
  # SSHクライアント側の死活監視。
  # 接続先の休止・ネットワーク断が起きると、TCPは切断を検知できないまま
  # クライアントが応答待ちで固まり、端末（Zellijのタブなど）ごと使えなくなる。
  # 暗号化チャネル上でキープアライブを送り、無応答なら約45秒で自ら切断する。
  # extraConfig は生成済みの `Host *` より前に置かれるため、
  # libvirt が挿入する `Host qemu/*` に巻き込まれないよう明示的にブロックで囲む
